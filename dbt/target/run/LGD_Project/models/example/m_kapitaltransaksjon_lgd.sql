  ...
  create or replace   view RISIKO.LGD.m_kapitaltransaksjon_lgd
...
/* Del 1: Transaksjoner direkte på kontoer i sak*/
 ...
 t.kapitaltransaksjon_beskrivelse
    from RISIKO.LGD.F_KAPITALTRANSAKSJON_T t
    join RISIKO.LGD.m_d_bankkunde_biii_lgd_t k on t.rk_bankkonto_id = k.rk_bankkonto_id
...
select
 ...
 t.kapitaltransaksjon_beskrivelse
  from RISIKO.LGD.F_KAPITALTRANSAKSJON_T t
  join RISIKO.LGD.m_d_bankkunde_biii_lgd_t k2 on k2.kontonummer = replace(substr(regexp_substr(t.kapitaltransaksjon_beskrivelse, 'FRA KTO \d{4}\.\d{2}\.\d{5}'), 9), '.', '')
                                        and t.tid_id between k2.sak_start_tid_id and k2.beregn_til_tid_id

  join RISIKO.LGD.M_KONFIGURASJON_BANK kb on kb.maletidspunkt_kode = k2.maletidspunkt_kode
...
select
 ...
 t.kapitaltransaksjon_beskrivelse
  from RISIKO.LGD.F_KAPITALTRANSAKSJON_T t
  join RISIKO.LGD.F_KNYTNING_SYNDIKAT ks on ks.rk_bankkonto_deltakerandel_id = t.rk_bankkonto_id
                                     and ks.tid_id = t.tid_id
  join RISIKO.LGD.m_d_bankkunde_biii_lgd_t k3 on k3.rk_bankkonto_id = ks.rk_bankkonto_hovedandel_id
                                        and t.tid_id between k3.sak_start_tid_id and k3.beregn_til_tid_id
  join RISIKO.LGD.M_KONFIGURASJON_BANK kb on kb.maletidspunkt_kode = k3.maletidspunkt_kode
