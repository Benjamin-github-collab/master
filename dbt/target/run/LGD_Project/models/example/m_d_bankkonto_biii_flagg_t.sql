        ...
        create or replace transient table RISIKO.LGD.m_d_bankkonto_biii_flagg_t
         as
        (/**********************************************************************************************
...
rk_trekkonto_utenfor_lgd as (
  select /*+ MATERIALIZE */ ku.sk_bankkunde_biii_id, ku.rk_bankkonto_id, ku.kontonummer,
    ...
    left join RISIKO.LGD.D_RESKONTRO_TREKKONTO rkt on rkt.kontonummer = ku.kontonummer
                                               and rkt.scd_gyldig_tom >= ku.sak_start_dato
                                               and rkt.scd_gyldig_fom <= ku.beregn_til_dato
    left join RISIKO.LGD.FAKE_D_BANKKONTO rkt_ak on rkt_ak.kontonummer = rkt.kontonummer_trekkonto
   where ku.tid_id = '20230331'
   and ku.batch_navn = 'batch_navn'
...
korr_trans_kilde as (
  select /*+ MATERIALIZE */ ku.sk_bankkunde_biii_id, ku.rk_bankkonto_id, ku.kontonummer,
    ...
    left join RISIKO.LGD.P_KORR_TRANS_KILDE_KODE p on p.rk_bankkunde_id = ku.rk_bankkunde_id
...
smn_bal_mangler_20180108 as (
  select /*+ MATERIALIZE */ ku.sk_bankkunde_biii_id, ku.rk_bankkonto_id, ku.kontonummer,
    ...
    left join RISIKO.LGD.F_KONTOBALANSE_T bal_mangler on bal_mangler.tid_id between '20180105' and '20180109'
...
ul_trans_mangler_20190826 as (
  select /*+ MATERIALIZE */ ku.sk_bankkunde_biii_id, ku.rk_bankkonto_id, ku.kontonummer,
    ...
    left join RISIKO.LGD.F_KONTOBALANSE_T bal_mangler on bal_mangler.tid_id between '20190823' and '20190827'
                                                  and bal_mangler.tid_id between ku.sak_start_tid_id and ku.beregn_til_tid_id
                                                  and bal_mangler.rk_bankkonto_id = ku.rk_bankkonto_id
   where ku.tid_id = '20230331'
   and ku.batch_navn = 'batch_navn'
