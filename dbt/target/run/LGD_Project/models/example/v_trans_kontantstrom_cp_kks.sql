  ...
  create or replace   view RISIKO.LGD.v_trans_kontantstrom_cp_kks
...
***********************************************************************************************/
tid as (
  select tid_id, dato, lag(tid_id) over (partition by 1 order by tid_id) tid_forrige_virkedag_id from RISIKO.LGD.D_TID where virkedag_flagg = '1'
),
cp_exit as (
  select c.bk_bankkonto_id, c.scd_slettet_i_kilde_dato
    from RISIKO.LGD.D_CASHPOOLHIERARKI c
...
select rk.bk_bankkonto_id, rk.scd_gyldig_fom, rk.scd_gyldig_tom
  from (select rk.rk_bankkonto_id,
               ...
               rk.scd_gyldig_tom,
               rk.scd_aktiv_flagg
          from RISIKO.LGD.D_RESKONTRO rk) rk
...
select tid.tid_id,
  ...
  join RISIKO.LGD.F_KONTOBALANSE_T b on b.tid_id = tid_forrige_virkedag_id
